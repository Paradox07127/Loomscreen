...
/// Metal-backed displacement map for rain-on-glass refraction.
final class RainGlassTexturePool: @unchecked Sendable {
    ...
    init(device: MTLDevice, inFlightTextureCount: Int = 3) {
        self.device = device
    ...
    func nextTexture(width: Int, height: Int) -> MTLTexture? {
        ...
        let requested = Dimensions(width: width, height: height)
        if dimensions != requested || textures.count != inFlightTextureCount {
    ...
    private func rebuild(width: Int, height: Int) {
        ...
        dimensions = textures.isEmpty ? nil : Dimensions(width: width, height: height)
        cursor = 0
    }
}
...
class RainGlassFilter: CIFilter, @unchecked Sendable {
    ...
    private struct MetalState {
        ...
        let texturePool: RainGlassTexturePool
    }
    ...
    private static let metalState: MetalState? = {
        ...
        return MetalState(
            commandQueue: queue,
            pipeline: pipeline,
            texturePool: RainGlassTexturePool(device: device)
        )
    }()

    override var outputImage: CIImage? {
        guard let inputImage = inputImage else { return nil }
        guard let state = Self.metalState else {
            return inputImage
        }
        ...
        guard let texture = state.texturePool.nextTexture(width: width, height: height) else {
            return inputImage
        }

        // Encode the compute pass.
        guard let buffer = state.commandQueue.makeCommandBuffer(),
        ...
        // Wait so CI samples a completed displacement texture; allocation is pooled.
        buffer.commit()
